...
export function TaskList({ tasks, selectedTaskId, onSelectTask, onAssignClick }: Props) {
  return (
    <div className="space-y-2">
      {tasks.map((t) => (
        ...
        >
          ...
          {t.assignedTo && (
            <div className="mt-2 text-xs text-muted-foreground">ผู้รับผิดชอบ: {t.assignedTo}</div>
          )}
        </div>
      ))}
    </div>
