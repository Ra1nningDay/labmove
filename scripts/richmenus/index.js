...
function buildAreas() {
  ...
  return [
    ...
    // สมัครสมาชิก (LIFF)
    {
      ...
      action: { type: 'uri', uri: `https://liff.line.me/${LIFF_ID}?mode=signup` },
    },
    // จองนัดเจาะเลือด (LIFF)
    {
      bounds: { x: 0, y: rowH, width: colW1, height: rowH },
      action: { type: 'uri', uri: `https://liff.line.me/${LIFF_ID}?mode=booking` },
    },
    // รายละเอียดนัด (postback)
    {
      bounds: { x: colW1, y: rowH, width: colW2, height: rowH },
      action: { type: 'postback', data: JSON.stringify({ action: 'booking_details' }) },
    },
    // โปรไฟล์ (postback)
    {
      bounds: { x: colW1 + colW2, y: rowH, width: colW1, height: rowH },
      action: { type: 'postback', data: JSON.stringify({ action: 'profile_show' }) },
    },
  ];
}
...
async function uploadImage(richMenuId) {
  const imgPath = path.join(process.cwd(), 'public', 'richmenu', 'richmenu.png');
...
async function list() {
  ...
  const alias = await lineFetch('https://api.line.me/v2/bot/richmenu/alias/list').catch(() => ({ aliases: [] }));
...
async function deleteAll() {
  ...
  for (const id of ids) {
    await lineFetch(`https://api.line.me/v2/bot/richmenu/${id}`, { method: 'DELETE' });
  }
...
async function deleteOthers(keepAlias = 'menu') {
  const alias = await lineFetch('https://api.line.me/v2/bot/richmenu/alias/list').catch(() => ({ aliases: [] }));
  const keep = (alias.aliases || []).find((a) => a.richMenuAliasId === keepAlias);
  const keepId = keep?.richMenuId;
  const res = await lineFetch('https://api.line.me/v2/bot/richmenu/list');
  const ids = (res.richmenus || []).map((m) => m.richMenuId);
  for (const id of ids) {
    if (id !== keepId) await lineFetch(`https://api.line.me/v2/bot/richmenu/${id}`, { method: 'DELETE' });
  }
  ...
  try {
    switch (cmd) {
      ...
      default:
        ...
        console.log('Commands: create-all | list | whoami | get <id> | upload-id <id> | delete-all | delete-others | set-default <id> | upload-image [alias]');
    }
  } catch (e) {
